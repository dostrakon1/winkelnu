-- Harden public offer ranking before the first live affiliate feed:
-- - unknown shipping must never be interpreted as confirmed free shipping;
-- - explicitly unavailable offers must not participate in public ranking;
-- - unknown availability stays eligible and is disclosed as unverified in the UI.

drop index if exists offers_active_last_seen_total_idx;
create index if not exists offers_active_last_seen_rank_idx
  on offers (product_id, last_seen_at desc, (shipping_cost is null), (price + coalesce(shipping_cost, 0)))
