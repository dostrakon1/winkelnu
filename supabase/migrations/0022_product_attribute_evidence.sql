-- M0.53 — Product attribute evidence for constraint-aware candidate ranking.
-- Keep raw, bounded label/value specifications with the canonical product so Search and
-- Vergelijkkompas can reason from the same evidence. Unknown data remains unknown.

alter table products
  add column if not exists specifications jsonb not null default '[]'::jsonb,
  add column if not exists visual_kind text;

alter table products drop constraint if exists products_specifications_is_array;
alter table products
  add constraint products_specifications_is_array
  check (jsonb_typeof(specifications) = 'array');

-- PostgreSQL cannot change a RETURNS TABLE shape through CREATE OR REPLACE, so recreate
-- the server-only ranking function with the two evidence fields included.
drop function if exists catalog_ranked_products(
  timestamptz, text, text, text, numeric, numeric, boolean, text, integer, integer
);

create function catalog_ranked_products(
  p_now timestamptz,
...
as $$
  with candidate_products as (
    ...
    where p.status = 'published'
      and (
        ...
        or parent_category.slug = p_category_slug
      )
    ...
    join lateral (
      ...
      where o.product_id = cp.id
        ...
        and lower(regexp_replace(btrim(coalesce(o.availability, '')), '[[:space:]-]+', '_', 'g')) not in (
          'out_of_stock', 'outofstock', 'not_in_stock', 'notinstock', 'unavailable',
          'sold_out', 'soldout', 'niet_op_voorraad', 'false', 'no', '0'
        )
      order by
        (o.shipping_cost is null) asc,
        o.price + coalesce(o.shipping_cost, 0) asc,
        o.id asc
      limit 1
    ) bo on true
    where cp.relevance >= 0
      and (
        not p_in_stock_only
        or lower(regexp_replace(btrim(coalesce(bo.availability, '')), '[[:space:]-]+', '_', 'g')) in (
          'in_stock', 'instock', 'available', 'op_voorraad', 'true', 'yes', '1'
        )
      )
  ...
  order by
    ...
    lower(r.title) asc,
    r.id asc
  limit greatest(1, least(p_limit, 97))
...
revoke all on function catalog_ranked_products(timestamptz, text, text, text, numeric, numeric, boolean, text, integer, integer) from anon, authenticated;
