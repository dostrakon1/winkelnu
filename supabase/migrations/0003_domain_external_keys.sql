...
alter table import_runs add column if not exists external_key text;
...
create unique index if not exists import_runs_external_key_uidx on import_runs(external_key) where external_key is not null;
...
comment on column import_runs.external_key is 'Stable application import-run identity independent from the relational UUID.';
