...
2. Apply migrations `0001` through `0015` in order.
...
8. Run `npm run check:db-contract`; CI statically verifies the full committed schema, critical operations security revoke/grant contracts and the idempotency retention policy.
...
24. Confirm `operator_action_requests` records are retained for at least 90 days; no automatic cleanup is enabled.
...
2. Apply exactly the same committed migrations `0001`–`0015`.
