...
import { Breadcrumbs, EditorialNotice, EditorialShell, GuideCard } from '@/components/storefront/editorial-shell'
...
export default async function BuyingGuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <EditorialShell>
      ...
      <article>
        ...
        <div className="wn-container grid gap-10 py-10 lg:grid-cols-[minmax(0,1fr)_17rem] lg:gap-14 lg:py-16">
          <div className="min-w-0">
            ...
            <section id="bronnen" className="mt-12 border-t border-[var(--wn-border)] pt-8"><h2 className="wn-heading text-2xl">Bronnen en werkwijze</h2><p className="wn-body-muted mt-4 text-sm leading-7">Deze gids is redactioneel opgesteld op basis van algemene productkennis en de onderstaande achtergrondbronnen. Het is geen eigen laboratoriumtest, een actuele productranglijst of een garantie dat een specifiek model aan jouw eisen voldoet. Controleer specificaties en voorwaarden altijd bij de fabrikant en verkoper.</p><ul className="mt-5 space-y-4">{guide.sources.map((source) => <li key={source.url}><a href={source.url} target="_blank" rel="noopener noreferrer" className="font-semibold text-[var(--wn-petrol)] underline underline-offset-4 hover:text-[var(--wn-petrol-deep)]">{source.title} ↗</a><p className="mt-1 text-sm leading-6 text-[var(--wn-text-muted)]">{source.note}</p></li>)}</ul></section>
          </div>
      ...
      <section className="wn-container pb-12"><EditorialNotice /></section>
