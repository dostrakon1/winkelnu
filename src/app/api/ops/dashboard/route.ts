...
export async function GET(request: Request) {
  ...
  if (process.env.CATALOG_PERSISTENCE !== 'supabase') {
    return Response.json({ ok: false, error: 'Operations dashboard requires CATALOG_PERSISTENCE=supabase.' }, {
      status: 503,
      headers: { 'Cache-Control': 'no-store' },
    })
  }
  ...
  const dashboard = buildOperationsDashboard(await service.read())
