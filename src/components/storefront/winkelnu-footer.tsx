        ...
        <div className="wn-container py-12 sm:py-16">
          <div className="grid gap-10 lg:grid-cols-[1.1fr_0.7fr_0.8fr_0.9fr]">
            <div className="max-w-xl">
              ...
              <p className="wn-display mt-5 text-2xl font-semibold tracking-[-0.025em]">Ontdek. Vergelijk. Kies je winkel.</p>
              <p className="mt-4 max-w-lg text-sm leading-6 text-white/75">
                Praktische keuzehulpen voor producten die bij jou passen. We werken aan een platform om aanbiedingen van verschillende webwinkels te vergelijken.
              </p>
            </div>
            ...
            <nav aria-label="Footer ontdekken">
              ...
              <div className="mt-4 flex flex-col items-start gap-3 text-sm text-white/80">
                <Link href="/lootje-lijstje" className="font-semibold text-[#ffb889] hover:text-white">✦ Lootje &amp; Lijstje</Link>
                <Link href="/koopgidsen" className="hover:text-white">Alle koopgidsen</Link>
                {editorialCategories.map((category) => (
                  <Link key={category.slug} href={getFooterBrowseHref(category)} className="hover:text-white">
                    {category.title}
                  </Link>
                ))}
                {catalogEnabled ? <Link href="/zoeken" className="hover:text-white">Producten vergelijken</Link> : null}
              </div>
            </nav>
            ...
            <nav aria-label="Footer informatie">
              ...
              <div className="mt-4 flex flex-col items-start gap-3 text-sm text-white/80">
                <Link href="/over-winkelnu" className="hover:text-white">Over Winkelnu</Link>
                <Link href="/affiliate-en-vergelijking" className="hover:text-white">Hoe Winkelnu werkt</Link>
                <Link href="/disclaimer" className="hover:text-white">Disclaimer</Link>
                <a href={`mailto:${operator.email}?subject=Winkelnu.nl`} className="hover:text-white">Contact opnemen</a>
              </div>
            </nav>
          ...
          <div className="mt-10 flex flex-col gap-3 border-t border-white/12 pt-6 text-xs leading-5 text-white/65 sm:flex-row sm:items-end sm:justify-between">
            <p>© {year} Winkelnu.nl · Akflow · KVK {operator.chamberOfCommerce}</p>
            <p className="max-w-xl sm:text-right">
              Winkelnu kan een vergoeding ontvangen via uitgaande links.{' '}
              <Link href="/affiliate-en-vergelijking" className="underline underline-offset-4 hover:text-white">Hoe werkt dat?</Link>
            </p>
          </div>
        </div>
      </footer>
