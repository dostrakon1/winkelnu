...
export function WinkelnuSurfaceMotif({ className = '' }: WinkelnuSurfaceMotifProps) {
  return (
    ...
    >
      ...
      <span className="absolute left-1/2 top-1/2 h-20 w-20 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-[var(--wn-petrol)]/70" />
      <span className="absolute left-1/2 top-1/2 h-px w-28 -translate-x-1/2 -translate-y-1/2 rotate-[-32deg] bg-[var(--wn-petrol)]/55" />
      <span className="absolute left-1/2 top-1/2 h-28 w-px -translate-x-1/2 -translate-y-1/2 rotate-[32deg] bg-[var(--wn-petrol)]/35" />
    </div>
  )
}
