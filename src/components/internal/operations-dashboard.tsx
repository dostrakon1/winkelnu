        ...
        <header className="flex flex-col gap-4 border-b border-slate-800 pb-8 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">
              Incidenten, veilige recovery en gecorreleerde operatorhistorie in één operationeel overzicht.
            </p>
          </div>
        ...
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_400px]">
          <section>
            <div className="mb-4">
              ...
              <p className="mt-1 text-sm text-slate-500">Per feed tonen we direct de laatste relevante recoveryhistorie.</p>
            </div>
                        ...
                        <div className="min-w-0">
                          ...
                          {relatedActions.length > 0 ? (
                            <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/40 p-3">
                              <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Recente recoverycontext</p>
                              <div className="mt-2 space-y-2">
                                {relatedActions.map((item) => (
                                  <div key={item.id} className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-400">
                                    <span className="font-medium text-slate-200">{actionLabel[item.action] ?? item.action}</span>
                                    <span>{item.actorEmail}</span>
                                    <span>{new Date(item.occurredAt).toLocaleString('nl-NL')}</span>
                                    <OutcomeBadge outcome={item.outcome} compact />
                                  </div>
                                ))}
                              </div>
                            </div>
          ...
          <section aria-label="Recente operatoracties">
            <div className="mb-4">
              ...
              <p className="mt-1 text-sm text-slate-500">Filter op actor, actie, outcome, merchant of feed.</p>
            </div>
            <form method="get" className="mb-4 grid gap-2 rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
              <input name="actor" defaultValue={historyFilters.actor ?? ''} placeholder="Actor e-mail" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200 placeholder:text-slate-600" />
              <select name="action" defaultValue={historyFilters.action ?? ''} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200">
                <option value="">Alle acties</option>
                <option value="feed.retry">Retry</option>
                <option value="feed.pause">Pauzeren</option>
                <option value="feed.resume">Hervatten</option>
              </select>
              <select name="outcome" defaultValue={historyFilters.outcome ?? ''} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200">
                <option value="">Alle outcomes</option>
                <option value="succeeded">Geslaagd</option>
                <option value="failed">Mislukt</option>
              </select>
              <input name="merchant" defaultValue={historyFilters.merchantId ?? ''} placeholder="Merchant ID" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200 placeholder:text-slate-600" />
              <input name="feed" defaultValue={historyFilters.sourceKey ?? ''} placeholder="Feed key" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200 placeholder:text-slate-600" />
              <div className="flex gap-2 pt-1">
                <button type="submit" className="rounded-lg border border-cyan-800 bg-cyan-950/40 px-3 py-2 text-xs font-semibold text-cyan-200 hover:border-cyan-600">Filter</button>
                <a href="/intern/operations" className="rounded-lg border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-400 hover:border-slate-500 hover:text-slate-200">Wissen</a>
              </div>
            </form>
            {actionHistory.length === 0 ? (
              <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5 text-sm text-slate-400">Geen operatoracties gevonden voor deze filters.</div>
            ) : (
              <div className="space-y-3">
...
function OutcomeBadge({ outcome, compact = false }: { outcome: OperatorActionHistoryItem['outcome']; compact?: boolean }) {
  return (
    <span className={`rounded-full border px-2 py-1 font-semibold ${compact ? 'text-[10px]' : 'text-[11px]'} ${outcome === 'succeeded' ? 'border-emerald-900 bg-emerald-950/40 text-emerald-300' : 'border-red-900 bg-red-950/40 text-red-300'}`}>
      {outcome === 'succeeded' ? 'Geslaagd' : 'Mislukt'}
    </span>
  )
}
